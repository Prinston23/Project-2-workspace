...
private SessionFactory sessionFactory;
	...
	public List<User> getSuggestedUsers(User user) {
		...
		SQLQuery query=session.createSQLQuery("select * from users where username in (select username from users where username!=? minus (select from_id from friend_new where to_id=?"
				+ "union select to_id from friend_new where from_id=?"
				+ "))");
	...
	public void updatePendingRequest(String from, String username, char status) {
		Session session=sessionFactory.openSession();
		SQLQuery query=session.createSQLQuery("update Friend_new set status=? where from_id=? and to_id=?");
	...
	public List<Friend> listOfFriends(String username){
		Session session=sessionFactory.openSession();
		SQLQuery query=session.createSQLQuery("select * from Friend_new where (from_id=? or to_id=?) and status=?");
